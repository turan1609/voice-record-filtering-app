...
def redirect_output():
    ...
    output_stream.new_text.connect(ui.plainText_Terminal.appendPlainText)  
...
def FILTERDATA():
    ...
    if not (turkish_selected or english_selected):
        QtWidgets.QMessageBox.warning(None, "Warning!", "Pick at least one Language.")
        return  # Fonksiyonu sonlandır

    # Cinsiyet kontrolü
    if not (male_selected or female_selected):
        QtWidgets.QMessageBox.warning(None, "Warning!", "Pick at least one Gender")
        return  # Fonksiyonu sonlandır
